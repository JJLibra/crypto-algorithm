...
class AES:
    ...
    def round_key_generator(self, _16bytes_key):
        # 轮密钥产生：
        # 将种子密钥分为四组，每组32位(4bytes)，分别对应w[0],w[1],w[2],w[3]
        w = [_16bytes_key >> 96,
    ...
    def aes_encrypt(self, plaintext_list, RoundKeys):
        ...
        State = self.AddRoundKey(State, RoundKeys, 0)  # 异或轮密钥，初始变换操作
        for Round in range(1, 10):
            State = self.SubBytes(State)  # 字节替换
            State = self.ShiftRows(State)  # 行移位
            State = self.MixColumns(State)  # 列混合
...
if __name__ == '__main__':
    aes = AES()
    key = 0x000102030405060708090a0b0c0d0e0f  # 子密钥Cipher Key
    RoundKeys = aes.round_key_generator(key)

    # 加密
    # plaintext = "AES_is_so_funny!"
    plaintext = 0x4145535F69735F736F5F66756E6E7921
    plaintext = aes.num_2_16bytes(plaintext)
    ciphertext = aes.aes_encrypt(plaintext, RoundKeys)
    print('ciphertext_hex = ' + hex(aes._16bytes2num(ciphertext)))

    # 解密
    ciphertext = 0xaef61da4530f248e9289eae044a936d8
    ciphertext = aes.num_2_16bytes(ciphertext)
    plaintext = aes.aes_decrypt(ciphertext, RoundKeys)
    print('plaintext_hex = ' + hex(aes._16bytes2num(plaintext)))
    # 转换为字符串
    print('plaintext = ' + bytes.fromhex(str(hex(aes._16bytes2num(plaintext)))[2:]).decode())
